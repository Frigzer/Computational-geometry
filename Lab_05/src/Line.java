...
public class Line implements Printable{
    ...
    public double angleBetweenLines(Line line) {
        /*double theta = Math.atan(Math.abs((line.a - a) / (1 + a * line.a)));
        ...
        Point2D.Double wektor1 = new Point2D.Double(p2.x - p1.x, p2.y - p1.y);
        Point2D.Double wektor2 = new Point2D.Double(line.p2.x - line.p1.x, line.p2.y - line.p1.y);

        // Iloczyn skalarny
        double ratio = wektor1.x * wektor2.x + wektor1.y * wektor2.y;

        // Długości wektorów
        double lenght1 = Math.sqrt(wektor1.x * wektor1.x + wektor1.y * wektor1.y);
        double lenght2 = Math.sqrt(wektor2.x * wektor2.x + wektor2.y * wektor2.y);

        // Obliczenie kąta
        double cosinusTheta = ratio / (lenght1 * lenght2);
        double theta = Math.acos(cosinusTheta);
